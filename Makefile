...
update_pfvar: $(CONFIGMAKE)
	@if ! $(cmd_CONFIGMAKE_RECURSE); then if $(TEST) "$(UNAME_SYS)" == "darwin"; then \
	     sys_xnu=`uname -a | $(SED) -e 's/.*:xnu-\([0-9.a-zA-Z]*\).*/\1/'`; \
	     echo "sys XNU: $${sys_xnu}"; \
	     xnu_list=`wget -O - https://opensource.apple.com/source/xnu | $(SED) -ne 's/.*>xnu-\([0-9a-zA-Z._]*\)\/*<.*/\1/p'`; \
	     serv_xnu=$${sys_xnu}; for ver in $${xnu_list}; do \
	         $(TEST) `echo | $(AWK) "{ print (\"$${ver}\" > \"$${sys_xnu}\"); }"` == "1" && break ; \
	         serv_xnu="$${ver}"; \
	     done; echo "serv XNU: $${serv_xnu}"; \
	     wget "https://opensource.apple.com/source/xnu/xnu-$${serv_xnu}/bsd/net/pfvar.h"; \
	     wget "https://opensource.apple.com/source/xnu/xnu-$${serv_xnu}/libkern/libkern/tree.h"; \
	 else \
	     echo "!! getting pfvar.h is only supported on macOS"; \
	 fi; fi
...
WARN_RELEASE	= -Wall -W -pedantic -Wno-extra-semi $(sys_WARN) # -Wno-ignored-attributes -Wno-attributes
...
$(BUILDDIRS): $(CONFIGMAKE) $(VERSIONINC) update-$(BUILDINC) .EXEC
...
$(CLANGCOMPLETE): $(ALLMAKEFILES) $(BUILDINC) $(CONFIGMAKE)
	@if ! $(cmd_CONFIGMAKE_RECURSE); then \
     ...
     $(AWK) 'BEGIN { ign=0 } /^[[:space:]]*$(DASH)[[:space:]]*auto-generated[[:space:]]/{ ign = 2 } \
             //{ if (ign>0) ign--; else print $$0 }' "$@" > "$@.tmp" $(NO_STDERR) || true; \
     for d in . $(SUBDIRS); do \
         short_d="$${d}"; \
